...
Процедура ВыгрузитьТовары(Команда)
    ...
    ПутьЗапроса_Создать = "api/admin/products";
	ТипЗапроса_Создать = "POST";

	ПутьЗапроса_Обновить = "api/admin/products/%1";
	ТипЗапроса_Обновить = "PUT";
	
	Для каждого стр из Товары Цикл
		Если НЕ стр.Выгружать Тогда
			Продолжить;
		КонецЕсли;
		Если стр.ВнешнийИД_Сайт = 0 Тогда
			ВызватьHTTPМетод(ПутьЗапроса_Создать, ТипЗапроса_Создать, ОписаниеПродукта(стр)); 
		Иначе	                                                              
			ВызватьHTTPМетод(ПутьЗапроса_Обновить, ТипЗапроса_Обновить, ОписаниеПродукта(стр)); 
		КонецЕсли;
...
&НаКлиенте
Процедура Товары_Удалить(Команда)

	ПутьЗапроса = "api/admin/products/%1";
	ТипЗапроса = "DELETE";
	...
	Для каждого стр из Товары Цикл
		...
		тПуть = СтрШаблон(ПутьЗапроса, стр.ВнешнийИД_Сайт); 
		ВызватьHTTPМетод(тПуть, ТипЗапроса, ОписаниеПродукта(стр));
	КонецЦикла;

КонецПроцедуры

&НаКлиенте
Процедура Товары_Обновить(Команда)
...
Функция ОписаниеПродукта(стрТаб)
	...
	Продукт.Вставить("ext_id", стрТаб.ИдСистемы);
...
Функция КатегорияКаталога(стрТаб)
	...
	РазделКаталога.Вставить("products_uid", НоменклатураВида(стрТаб.ВидНоменклатуры));
...
Функция НоменклатураВида(ВидНоменклатуры) 
	...
	Пока Выборка.Следующий() Цикл	
		Состав.Добавить(Строка(Выборка.уид));
	КонецЦикла;
